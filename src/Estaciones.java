...
public class Estaciones implements Runnable {
    ...
    static protected AtomicBoolean[][] stationReady;
    ...
    public Estaciones(int ROW, int station, Queue<Robot>[] availableRobots) {
        this.ROW = ROW;
        this.STATION = station;

        if (stationReady == null) {
            Estaciones.availableRobots = availableRobots;
            init();
    ...
    private void init() {
        ...
        stationReady = new AtomicBoolean[app2.ROWS][app2.STATIONS];

        for (int i = 0; i < app2.ROWS; i++) {
            stationReady[i][0] = new AtomicBoolean(true);
            car[i][0] = -1;
        }
        ...
        for (int i = 0; i < app2.ROWS; i++) {
            for (int j = 1; j < app2.STATIONS; j++) {
                car[i][j] = -1;
                stationReady[i][j] = new AtomicBoolean(false);
            }
        }
    ...
    protected void work() throws InterruptedException {
        ...
        if (STATION == 0) {
            ...
            carSem.libera();
            // System.out.println("Produciendo carro " + (car[ROW][STATION] + 1));
        }
